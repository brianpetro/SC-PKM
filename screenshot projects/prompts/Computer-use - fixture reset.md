...
prompt_id: screenshot-fixture-reset
mode: FIXTURE_RESET
input_record_types:
  - scene
  - fixture
output_record_types:
  ...
  - decision
allowed_write_roots:
  - screenshot projects/runs/fixture
  - fixture root supplied by dispatch
...
- a scene requires a named fixture snapshot
- the fixture may have drifted
...
Restore or validate one exact screenshot fixture state and profile so the linked scene can be verified without semantic contamination or hidden tier leakage.
...
Create before changing the fixture:

```text
screenshot projects/runs/fixture/{reset_id}.md
```
...
- plugin source code
- content outside the fixture root
...
- uncommitted or unexpected fixture changes
- current index state

Do not overwrite unexpected user-authored material.

WHEN content exists inside the allowed fixture root but is not part of the named fixture and cannot be safely isolated THEN mark the run `needs-review`.

### 4. Apply the dispatched reset strategy

#### restore

- Locate the materialized `{{target_snapshot}}` artifact.
- Restore it exactly into `{{allowed_fixture_root}}`.
...
- Create the fixture only from the canonical fixture note.
- Preserve exact paths, titles, prose, Base invariants, and negative controls.
- Create a checksum manifest for all fixture-controlled files.
...
Restore only settings required by the fixture and linked scene, including when applicable:
...
Do not tune semantic algorithms, feedback, filters, or ranking unless the canonical fixture/profile contract explicitly requires them.

### 6. Rebuild or refresh the index

- Use the minimum current product action that makes the restored fixture current.
...
- no duplicate-titled fixture files
...
```md
fixture run `failed` or `needs-review`
